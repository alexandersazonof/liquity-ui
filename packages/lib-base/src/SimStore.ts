...
export interface SimStoreBaseState {
   ...
   * {@link SimStoreDerivedState.trove | trove}.
   */
...
/**
 * State variables derived from {@link SimStoreBaseState}.
 *
 * @public
 */
...
/**
 * Type of {@link SimStore}'s {@link SimStore.state | state}.
 *
 * @remarks
 * It combines all properties of {@link SimStoreBaseState} and {@link SimStoreDerivedState}
 * with optional extra state added by the particular `SimStore` implementation.
 *
 * The type parameter `T` may be used to type the extra state.
 *
 * @public
 */
export type SimStoreState<T = unknown> = SimStoreBaseState & SimStoreDerivedState & T;

/**
 * Parameters passed to {@link SimStore} listeners.
 *
 * @remarks
 * Use the {@link SimStore.subscribe | subscribe()} function to register a listener.
 ...
 *
 * @remarks
 * The type parameter `T` may be used to type extra state added to {@link SimStoreState} by the
 * subclass.
 *
 * Implemented by {@link @sim/lib-ethers#BlockPolledSimStore}.
 *
 * @public
 */
export abstract class SimStore<T = unknown> {
  ...
  private _baseState?: SimStoreBaseState;
  private _derivedState?: SimStoreDerivedState;
  private _extraState?: T;

  private _updateTimeoutId: ReturnType<typeof setTimeout> | undefined;
  private _listeners = new Set<(params: SimStoreListenerParams<T>) => void>();
   ...
   * {@link SimStore.onLoaded | onLoaded} to get a callback when this happens.
   *
   * See {@link SimStoreState} for the list of properties returned.
   */
   ...
   *
   * @remarks
   * The {@link SimStore.onLoaded | onLoaded} callback will be called after the state is fetched
   * for the first time.
   *
   * Use the {@link SimStore.subscribe | subscribe()} function to register listeners.
   *
  ...
  }: SimStoreBaseState): SimStoreDerivedState {
  ...
  private _reduceDerived(
    derivedState: SimStoreDerivedState,
    derivedStateUpdate: SimStoreDerivedState
  ): SimStoreDerivedState {
    return {
  ...
  private _notify(params: SimStoreListenerParams<T>) {
  ...
  /** @internal */
  protected _load(baseState: SimStoreBaseState, extraState?: T): void {
    assert(!this._loaded);
